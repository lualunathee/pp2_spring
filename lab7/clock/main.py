...
clock = pygame.time.Clock() #для частоты кадров
...
rect = bg.get_rect(center=(415, 418)) #получает прямоугольник изображения и центрирует его
...
while not done:
 ...
 #будет продолжатся пока игрок не закроет окно
    time = datetime.datetime.now()
    
    pressed = pygame.key.get_pressed()
    
    if pressed[pygame.K_BACKSPACE] and pressed[pygame.K_LEFT]: #меняет фон на другое изображение
